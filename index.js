...
function startPriceTracking(userId, tokenAddress, interval) {
  ...
  trackers.push({ tokenAddress, intervalId, mode: 'price', interval });
  return true;
}
...
⏰ ${new Date().toLocaleTimeString('ru-RU')}
  ...
  trackers.push({ tokenAddress, intervalId, mode: 'alert', checkInterval, threshold });
  return true;
}
...
bot.action(/^mode_price_(.+)$/, async (ctx) => {
...
bot.action(/^mode_alert_(.+)$/, async (ctx) => {
  await ctx.answerCbQuery();
  const address = ctx.match[1];
...
// ===============================
//   ПОРОГ → ИНТЕРВАЛЫ ALERT
// ===============================

bot.action(/^threshold_(.+)_(\d+)$/, async (ctx) => {
  await ctx.answerCbQuery();
  const address = ctx.match[1];
...
bot.action(/^check_interval_(.+)_(\d+)_(\d+)$/, async (ctx) => {
  await ctx.answerCbQuery();
  const address = ctx.match[1];
  const threshold = parseInt(ctx.match[2]);
  ...
  if (!ok) return ctx.answerCbQuery("❌ Можно отслеживать максимум 5 монет");
...
bot.action(/^interval_price_(.+)_(\d+)$/, async (ctx) => {
  await ctx.answerCbQuery();
  const address = ctx.match[1];
...
bot.action(/^stop_(.+)$/, async (ctx) => {
  await ctx.answerCbQuery('⏹ Остановлено');
