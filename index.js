...
async function getTokenPrice(address) {
  try {
    ...
    const token = response.data?.included?.find(i => i.type === "token")?.attributes;

    if (!pool || !token) return null;

    return {
      price: pool.token_price_usd,
      change24h: pool.price_change_percentage?.h24,
      volume24h: pool.volume_usd?.h24,
...
💎 Ликвидность: $${(data.liquidity / 1000000).toFixed(2)}M
...
bot.start((ctx) => {
  ctx.reply(
    ...
    '📊 Отправляю цену мемкойна каждые 30 сек — 5 мин\n\n' +
