...
// userId → { step, address, mode, threshold, interval }
...
function resetUserState(ctx) {
  ...
  ctx.reply("🔄 Действие отменено. Введи адрес токена заново.");
}
...
bot.start((ctx) => {
  ctx.reply(
    ...
    "• порог (%)\n" +
    "• интервал (мс)\n\n" +
    "🔥 Без кнопок. Всё вручную. Работает идеально.",
    { parse_mode: "Markdown" }
  );
});
...
bot.command("cancel", (ctx) => resetUserState(ctx));
...
bot.command("stop", (ctx) => {
  ...
  ctx.reply("⏹ Все отслеживания остановлены. Диалог сброшен.");
});
...
bot.on("text", async (ctx) => {
  ...
  // Автоотмена
  if (["отмена", "cancel", "стоп", "stop", "назад"].includes(msg)) {
    return resetUserState(ctx);
  }
...
// ===============================
//   ЗАПУСК БОТА (POLLING + KEEPALIVE)
// ===============================

setInterval(() => console.log("💓 keepalive"), 20000);

bot.launch();
console.log("🚀 Bot запущен (текстовый режим, Railway-friendly)");
